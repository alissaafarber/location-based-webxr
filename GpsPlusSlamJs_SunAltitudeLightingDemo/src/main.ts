...
scene.background = new THREE.Color(0x87ceeb); // Sky blue — overridden each frame by lighting model
...
// Ambient light — intensity driven by sunAltitudeToLighting().ambientLevel
...
// Lighting status display — showcases sunAltitudeToLighting() output
const lightingDiv = document.createElement('div');
lightingDiv.style.marginTop = '15px';
lightingDiv.style.paddingTop = '12px';
lightingDiv.style.borderTop = '1px solid #ccc';
lightingDiv.innerHTML = `
  <h3 style="margin: 0 0 10px 0;">Lighting</h3>
  <div id="lightingStatus">
    <div>Phase: <span id="lightingPhase">--</span></div>
    <div>Intensity: <span id="lightingIntensity">--</span></div>
    <div>Color: <span id="lightingColor">--</span>
      <span id="lightingColorSwatch" style="
    ...
    <div>Ambient Level: <span id="lightingAmbient">--</span></div>
  </div>
`;
...
const lightingPhaseDisplay = document.getElementById('lightingPhase') as HTMLSpanElement;
const lightingIntensityDisplay = document.getElementById('lightingIntensity') as HTMLSpanElement;
const lightingColorDisplay = document.getElementById('lightingColor') as HTMLSpanElement;
const lightingColorSwatch = document.getElementById('lightingColorSwatch') as HTMLSpanElement;
const lightingAmbientDisplay = document.getElementById('lightingAmbient') as HTMLSpanElement;

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

/** Infer a human-readable phase label from sun altitude in degrees. */
function lightingPhaseName(altitudeDeg: number): string {
  if (altitudeDeg < -18) return 'Night';
  if (altitudeDeg < -6)  return 'Astronomical twilight';
  if (altitudeDeg < 0)   return 'Civil twilight';
  if (altitudeDeg < 6)   return 'Golden hour';
  return 'Daytime';
}
...
// ---------------------------------------------------------------------------
// Update sun position, shadow rig, and lighting model
// ---------------------------------------------------------------------------
function updateSunPosition() {
  try {
    ...
    // --- Lighting model ---------------------------------------------------
    const lighting = sunAltitudeToLighting(result.altitudeRad);

    // Apply lighting to shadow rig's directional light (also drives shadow visibility)
    sunShadowRig.update(result.directionNue, undefined, lighting);

    // Drive the directional light color and intensity from the lighting model
    sunShadowRig.directionalLight.color.setHex(lighting.color);
    sunShadowRig.directionalLight.intensity = lighting.intensity;

    // Drive ambient light from the lighting model
    ambientLight.intensity = lighting.ambientLevel;

    // Update scene background to match the atmospheric color at current altitude
    // Scale the light color toward black for night / toward sky-blue for day
    const r = ((lighting.color >> 16) & 0xff) / 255;
    const g = ((lighting.color >> 8)  & 0xff) / 255;
    const b =  (lighting.color        & 0xff) / 255;
    const bgScale = lighting.ambientLevel * 0.45;
    scene.background = new THREE.Color(r * bgScale, g * bgScale, b * bgScale);

    // --- Sun status display -----------------------------------------------
    const altitudeDeg = result.altitudeRad * (180 / Math.PI);
    const azimuthDeg  = result.azimuthRad  * (180 / Math.PI);
    ...
    // --- Lighting display -------------------------------------------------
    const colorHex = `#${lighting.color.toString(16).padStart(6, '0')}`;
    lightingPhaseDisplay.textContent     = lightingPhaseName(altitudeDeg);
    lightingIntensityDisplay.textContent = lighting.intensity.toFixed(3);
    lightingColorDisplay.textContent     = colorHex;
    lightingColorSwatch.style.background = colorHex;
    lightingAmbientDisplay.textContent   = lighting.ambientLevel.toFixed(3);
    ...
    lightingPhaseDisplay.textContent     = 'Error';
    lightingIntensityDisplay.textContent = 'Error';
    lightingColorDisplay.textContent     = 'Error';
    lightingAmbientDisplay.textContent   = 'Error';
  }
}
